...
float func(vec2 p) {
    float n = tetraNoise(vec3(p.x*1.0, p.y*1.0, 0.0) - vec3(0.0, 0.25, 0.5)*time);
    float taper = 0.1 + dot(p, p*vec2(0.35, 1.0));
    n = max(n - taper, 0.0)/max(1.0 - taper, 0.00001);
    ns = n; 
    const float palNum = 25.0;
...
void main() {
    ...
    for (int i = startingLayers; i < maxLayers; i++) {
        if (i >= layers) break;
        float fi = float(i);
        vec2 q = uvCentered*(1.0+fi*DEPTH*0.5);
    ...
    vec3 effect = acc * 0.5 * (0.6 + (coolNoise() * 0.2));
    effect *= (pixely ? 1.6 : 1.0) * BRIGHT * 0.7;
