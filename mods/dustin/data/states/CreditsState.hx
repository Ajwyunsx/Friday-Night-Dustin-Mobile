...
function update(elapsed:Float) {
    if (FlxG.keys.justPressed.SPACE && lineText.isTyping) {
        lineText.skip();
    }
    ...
    if (FlxG.keys.justPressed.ESCAPE && !isAnimating) {
        if (infoVisible) {
    ...
    if (infoVisible && (FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.RIGHT)) {
